#' module_water_L1233.Elec_water_India
#'
#' Downscaled electricity generation, water withdrawals and water consumption by basin in India.
#'
...
module_water_L1233.Elec_water_India <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    ## STEP 6A: PARTITION FUEL INPUTS TO ELECTRICITY GENERATION...
    ...
    #ZKADDITION STEP 6B: DOWNSCALING ANNUAL ELECTRICITY GENERATION TO THE BASIN LEVEL
    #...UTILISING WITHDRAWALS SHARES FROM THE SHARE FILES BELOW SINCE
    #...THAT IS THE AMOUNT BEING WITHDRAWN
    ...
    L1233.out_EJ_R_elec_F_tech_Yh_cool %>%
      left_join(L103.water_mapping_R_B_W_wdraw_share,
                by =  c("GCAM_region_ID"
                )) %>%
      mutate(value = value * share) %>%
      select(- water_sector, - share) %>%
      filter("GCAM_region_ID" == "India")->
      L1233.out_EJ_R_elec_B_F_tech_Yh_cool
    L1233.out_EJ_R_elec_B_F_tech_Yh_cool <- L1233.out_EJ_R_elec_B_F_tech_Yh_cool[c("GCAM_region_ID", "GCAM_basin_ID", "sector",
                                                                                   "fuel", "technology", "cooling_system", "water_type",
                                                                                   "plant_type", "year", "value")]
    ...
    L1233.out_EJ_R_elec_B_F_tech_Yh_cool%>%
      add_title("Electricity output by region, basin, fuel, technology, cooling system, and water type") %>%
      add_units("EJ") %>%
      add_comments("Computed by multiplying region level electricity generation by basin-level electricity generation at the technology-level output") %>%
      add_legacy_name("L1233.out_EJ_R_elec_B_F_tech_Yh_cool") %>%
    ...
    return_data(L1233.out_EJ_R_elec_F_tech_Yh_cool, L1233.out_EJ_R_elec_B_F_tech_Yh_cool, L1233.in_EJ_R_elec_F_tech_Yh_cool, L1233.wdraw_km3_R_elec, L1233.wcons_km3_R_elec, L1233.shrwt_R_elec_cool_Yf,
